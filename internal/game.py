...
def valid_position(grid: list[list[int]], block: list[list[int]], i: int, j: int) -> bool:
    ...
    if (i + block_size[0] > len(grid[0]) # if x + block width > grid width
            or j > len(grid) # or y > grid height
            or i < 0 # or x < 0
            or j - block_size[1] + 1 < 0): # or y - block height + 1 < 0
        return False
    ...
    for k in range(block_size[1]):
        ...
        for l in range(block_size[0]):
            ...
            if grid[y][x] not in [1] and truncated[k][l]:
                return False
